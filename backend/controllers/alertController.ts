...
export const sendLowStockAlert = async (req: Request, res: Response) => {
  try {
    ...
    if (!products || products.length === 0) return res.status(400).json({ message: "No products provided" });

    const staff = await Staff.find({ role: { $in: ["admin","product_manager","sales"] }}).select("email");
    const emails = staff.map(s => s.email);
    if (!emails.length) return res.status(400).json({ message: "No staff emails found" });

    const productLines = products.map((p: { name: any; brand: any; stock: any; }) => `• ${p.name} (${p.brand}) - Only ${p.stock} left`).join("\n");
